...
mAuth.signInWithEmailAndPassword(Email,Pw)
    ...
    private String getErrorMessage(Exception exception){
        ...
        if(err!=null){
            if(err.contains("WRONG_PASSWORD")){
            ...
            if(err.contains("USER_DISABLED")){
                return "Tài khoản đã bị vô hiệu hóa";
            }
            if(err.contains("TOO_MANY_REQUESTS")){
                return "Quá nhiều lần đăng nhập";
            }

        }
        return"Đăng nhập thất bại. Vui lòng thử lại sao";
    }
    private void navigateToProfile(FirebaseUser user) {
       if (user != null) {
           Intent intent=new Intent(this,com.example.myapplication.Profile.ThongTinNDActivity.class);
